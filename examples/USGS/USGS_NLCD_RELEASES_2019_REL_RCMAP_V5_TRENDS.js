// Import the NLCD RCMAP TRENDS.
var dataset = ee.Image('projects/ee-rcmap/assets/RCMAP_V5_TRENDS/TRENDS');
// DO NOT SUBMIT - Enable the final location
// var dataset = ee.ImageCollection('USGS/NLCD_RELEASES/2019_REL/RCMAP/V5/TRENDS');

print('Bands:', dataset.bandNames());

// Demonstrate using regular expressions to select bands.
var bareGround = dataset.select('bare_ground.*');
print('"bare_ground', bareGround, bareGround.bandNames());

var pvalue = dataset.select('.*linear_model_pvalue');
print('linear_model_pvalue', pvalue, pvalue.bandNames());

var percentCover = dataset.select('annual_herbaceous_break_point');
...
Map.addLayer(percentCover, vis, 'annual herbaceous break point');