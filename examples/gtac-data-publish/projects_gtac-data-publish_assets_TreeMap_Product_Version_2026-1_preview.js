...
var classValues = ee.String(
    treemap.get('FORTYPCD_class_values')
).split(',').map(function(value) {
  return ee.Number.parse(value);
});

var classPalette = ee.String(
    treemap.get('FORTYPCD_class_palette')
).split(',');

var paletteIndexes = ee.List.sequence(
    0, classValues.length().subtract(1));

var image = fortypcd
    .remap(classValues, paletteIndexes)
    .visualize({
      min: 0,
      max: classValues.length().subtract(1),
