local id = 'COPERNICUS/MARINE/INSITU_MULTI_4KM/RADAR_RADIAL';
local insitu = import 'templates/copernicus_marine_insitu.libsonnet';
local ee_const = import 'earthengine_const.libsonnet';
local ee = import 'earthengine.libsonnet';

local basename = std.strReplace(id, '/', '_');
local self_ee_catalog_url = ee_const.ee_catalog_url + basename;

{
  stac_version: ee_const.stac_version,
  type: ee_const.stac_type.collection,
  id: id,
  title: 'Copernicus Global In-situ Observations of Ocean Currents - Radar Radial',
  'gee:type': ee_const.gee_type.bigquery_table,
  'gee:bq_table_name': 'earth-engine-public-data.insitu_nrt_currents.radar_radial_latest',
  description: insitu.description + |||
    This dataset contains sea surface current observations from High-Frequency
    (HF) radars. Sea surface currents measured by HF radar are an average over
    the top 0.3-2.5 m of the water column, depending on the operating central
    frequency.

    This dataset provides radial velocity components. It comprises gridded maps
    of radial velocity fields of the surface current averaged over a time
    interval and delivered on an hourly basis. Radial velocities are measured
    on a polar grid and then remapped onto a Cartesian grid. The final product
    is a map of the zonal and meridional components of the radial ocean currents
    on a regular grid in the area covered by the individual radar stations.
  |||,
  license: insitu.license.id,
  links: ee.standardLinks(insitu.subdir, id),
  'gee:categories': ['oceans'],
  keywords: insitu.keywords + ['radar'],
  providers: insitu.providers(self_ee_catalog_url),
  extent: ee.extent_global('2025-03-28T00:00:00Z', null),
  summaries: {
    'gee:schema': [
      {
        name: 'TIME',
        description: 'Datetime of the observation',
        type: ee_const.var_type.datetime,
      },
      {
        name: 'LATITUDE',
        description: 'Latitude of the observation',
        type: ee_const.var_type.double,
      },
      {
        name: 'LONGITUDE',
        description: 'Longitude of the observation',
        type: ee_const.var_type.double,
      },
      {
        name: 'DEPTH',
        description: 'Number of depth levels',
        type: ee_const.var_type.double,
      },
      {
        name: 'RNGE',
        description: 'Range (away from the instrument) of the measurement (km)',
        type: ee_const.var_type.double,
      },
      {
        name: 'BEAR',
        description: |||
          Bearing (away from the instrument) of the measurement (degrees)
        |||,
        type: ee_const.var_type.double,
      },
      {
        name: 'DEPH',
        description: 'Depth of the observation (m)',
        type: ee_const.var_type.double,
      },
      {
        name: 'ESPC',
        description: |||
          Radial Standard Deviation of Current Velocity over the Scatter
          Patch (m/s)
        |||,
        type: ee_const.var_type.double,
      },
      {
        name: 'ETMP',
        description: |||
          Radial Standard Deviation of Current Velocity over the Coverage
          Period (m/s)
        |||,
        type: ee_const.var_type.double,
      },
      {
        name: 'MAXV',
        description: 'Maximum velocity',
        type: ee_const.var_type.double,
      },
      {
        name: 'MINV',
        description: 'Minimum velocity',
        type: ee_const.var_type.double,
      },
      {
        name: 'ERSC',
        description: 'Radial surface current error',
        type: ee_const.var_type.double,
      },
      {
        name: 'ERTC',
        description: 'Total radial current error',
        type: ee_const.var_type.double,
      },
      {
        name: 'XDST',
        description: 'Distance in X-direction from radar site',
        type: ee_const.var_type.double,
      },
      {
        name: 'YDST',
        description: 'Distance in Y-direction from radar site',
        type: ee_const.var_type.double,
      },
      {
        name: 'SPRC',
        description: 'Spectral peakiness',
        type: ee_const.var_type.double,
      },
      {
        name: 'OWTR_QC',
        description: insitu.quality_control_flag_description('OWTR'),
        type: ee_const.var_type.int,
      },
      {
        name: 'CSPD_QC',
        description: insitu.quality_control_flag_description('CSPD'),
        type: ee_const.var_type.int,
      },
      {
        name: 'VART_QC',
        description: insitu.quality_control_flag_description('VART'),
        type: ee_const.var_type.int,
      },
      {
        name: 'MDFL_QC',
        description: insitu.quality_control_flag_description('MDFL'),
        type: ee_const.var_type.int,
      },
      {
        name: 'AVRB_QC',
        description: insitu.quality_control_flag_description('AVRB'),
        type: ee_const.var_type.int,
      },
      {
        name: 'RDCT_QC',
        description: insitu.quality_control_flag_description('RDCT'),
        type: ee_const.var_type.int,
      },
      {
        name: 'QCflag',
        description: 'Overall quality flag',
        type: ee_const.var_type.int,
      },
      {
        name: 'DRVA',
        description: |||
          Direction of Radial Sea Water Velocity away From Instrument (degrees)
        |||,
        type: ee_const.var_type.double,
      },
      {
        name: 'RDVA',
        description: |||
          Radial Sea Water Velocity Away From Instrument (m/s)
        |||,
        type: ee_const.var_type.double,
      },
      {
        name: 'EWCT',
        description: 'Surface Eastward Sea Water Velocity (m/s)',
        type: ee_const.var_type.double,
      },
      {
        name: 'NSCT',
        description: 'Surface Northward Sea Water Velocity (m/s)',
        type: ee_const.var_type.double,
      },
      {
        name: 'POSITION_QC',
        description: insitu.quality_control_flag_description('POSITION'),
        type: ee_const.var_type.int,
      },
      {
        name: 'SDN_STATION',
        description: 'Station identifier',
        type: ee_const.var_type.string,
      },
      {
        name: 'TIME_QC',
        description: insitu.quality_control_flag_description('TIME'),
        type: ee_const.var_type.int,
      },
      {
        name: 'DEPH_QC',
        description: insitu.quality_control_flag_description('DEPH'),
        type: ee_const.var_type.int,
      },
      {
        name: 'geometry',
        description: 'Geographic location of the observation',
        type: ee_const.var_type.geometry,
      },
    ],
    'gee:visualizations': [
      {
        display_name: 'Global INSITU Radar Radial RDVA',
        image_visualization: {
          band_names: ['RDVA'],
          min: -1,
          max: 1,
          palette: ['a50026', 'd73027', '333333', '4575b4', '313695'],
        },
        lookat: {
          lon: -9.46,
          lat: 42.76,
          zoom: 8,
        },
      },
    ],
  },
  'gee:skip_featureview_generation': true,
  'gee:terms_of_use': insitu.terms_of_use,
}
