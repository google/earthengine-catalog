local id = 'FAO/GAUL_SIMPLIFIED_500m/2025/level2';
local subdir = 'FAO';
local versions = import 'versions.libsonnet';
local version_table = import 'templates/GAUL_SIMPLIFIED_500m_level2_versions.libsonnet';
...

{
  ...
  id: id,
  title:
    'FAO GAUL 500m Simplified: Global Administrative Unit Layers 2025, ' +
  ...
  description: |||
    ...
    The administrative boundaries dataset at level 2 (sub-national level) is part
    ...
    The administrative boundaries at the level 2 dataset distinguishes Districts,
    Counties, Municipalities and equivalents.
  ...
  extent: ee.extent_global('2025-01-01T00:00:00Z', '2025-12-31T00:00:00Z'),
  summaries: {
    'gee:schema': [
      {
        name: 'FID',
  ...
  |||,
  'gee:terms_of_use': |||
    This adaptation was not created by the Food and Agriculture Organization of the United Nations (FAO). FAO is not responsible for the content or accuracy of this adaptation. The original English edition shall be the authoritative edition.
  ||| + ee.gee_terms_of_use(license),
}
