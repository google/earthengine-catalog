...
local vertical_column_sample_bands_list = [
...
local vertical_column_sample_bands() = [
  local type_map = {'min': 'Smallest ', 'max': 'Largest ', 'num': 'Number of '};
  local kind_map = {
  ...
  };
  {
    ...
    description: std.format(
      |||
        %sLevel 2 pixel values contributing to Level 3 grid for the
        %s vertical column%s
      |||,
      [
        type_map[std.split(band, '_')[0]],
        kind_map[std.split(band, '_')[3]],
        if std.member(band, 'uncertainty') then ' uncertainty.' else '.',
      ]
    ),
    'gee:units': if std.member(band, 'num')
      then units.dimensionless
      else units.molecules_per_cm2,
  }
  for band in vertical_column_sample_bands_list
];

{
  nasa_tempo_no2_collection(cfg) ::
    ...
    local extra_bands = if std.objectHas(cfg, 'extra_bands') then cfg.extra_bands else [];
    {
      ...
      id: cfg.id,
      title: 'TEMPO gridded NO2' + cfg.qa_title +
        ' tropospheric and stratospheric columns ' + version +
      ...
      'sci:citation': |||
        ...
        stratospheric columns %s (PROVISIONAL) [Data set]. NASA Langley
