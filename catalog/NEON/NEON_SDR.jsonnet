...

{
  ...
  description: |||
    Orthorectified surface directional reflectance (0-1 unitless, scaled by
    10000) computed from one of three NEON Imaging Spectrometer (NIS) units,
    AVIRIS-NG sensors built by NASA's Jet Propulsion Lab (JPL).  This
    hyperspectral dataset contains 426 bands spanning wavelengths from ~380 nm
    to 2510 nm.  Wavelengths between 1340-1445nm and 1790-1955nm are set to
    -100; these are water vapor absorption bands without valid values. Spatial
    resolution is 1m and spectral resolution is ~5nm.  A Bidirectional
    Reflectance Distribution Function (BRDF) correction has not been applied.
    Flight lines over a given site are mosaicked using the highest quality cloud
    conditions and closest proximity to nadir. Data are calibrated and
    atmospherically corrected, and include QA and ancillary bands comprising of
    inputs to and outputs from the Atmospheric & Topographic Correction (ATCOR),
    weather quality information, and the acquisition date; see the "Bands" tab
    for more detailed information.

    Documentation: [NEON DP3.30006.001 Quick Start
    ...
    'gee:schema': [
      {
        ...
        name: 'SENSOR_NAME',
        description:
          'Make and model of the hyperspectral sensor; ' +
          'all NEON spectrometers are NASA ' +
          'Jet Propulsion Lab (JPL) AVIRIS-NG sensors',
        type: ee_const.var_type.string,
      }, {
        name: 'SENSOR_NUMBER',
        description:
          'Number of the NEON Imaging Spectrometer (NIS), ' +
          'corresponding to the payload flown: ' +
          '"NIS1", "NIS2", "NIS3", "CAO" (Carnegie Airborne Observatory), or ' +
    ...
    'eo:bands': [
      {
        ...
        description:
          ...
          'sea level values of visibility index / total optical thickeness',
      }, {
        name: 'B438',
        'gee:units': units.centimeter,
        description:
          'Water Vapor Column - cm * 1000 ground-to-space used in ATCOR',
        // TODO(bhass-neon): Should this just be hectometer?
        // Or it needs a gee:scale
      }, {
        name: 'B439',
        'gee:units': units.degree,
        ...
        description:
          ...
          'estimated percentage of overhead cloud cover during acquisition',
        'gee:classes': [
          {
            value: 0,
            color: '239e2f',
        ...
        name:'B442',
        // TODO(schwehr): 'gee:units': 'Flight Date',
        description: 'Acquisition Date, YYYYMMDD',
      },
    ],
    'gee:visualizations': [{
        display_name: 'Surface Directional Reflectance (SDR)',
  ...
  'gee:terms_of_use': ee.gee_terms_of_use(license) + '\n\n' + |||
    ...
    CC0 1.0 "No Rights
    Reserved"](https://creativecommons.org/publicdomain/zero/1.0/).
    No copyright has been applied to NEON data; any person may copy,
    modify, or distribute the data, for commercial or non-commercial
    purposes, without asking for permission. NEON data may still be
    subject to other laws or rights such as for privacy, and NEON
    makes no warranties about the data and disclaims all
    liability. When using or citing NEON data, no implication should
    be made about endorsement by NEON. In most countries, data and
    facts are not copyrightable.  By putting NEON data into the public
    domain, we encourage broad use, particularly in scientific
    analyses and data aggregations. However, please be aware of the
    following scholarly norms: NEON data should be used in a way that
    is mindful of the limitations of the data, using the documentation
    associated with the data packages as a guide. Please refer to
    [NEON Data Guidelines and
    Policies](https://www.neonscience.org/data-samples/guidelines-policies)
    for detailed information on how to properly use and cite NEON
    data, as well as best practices for publishing research that uses
    NEON data.
  |||,
}
