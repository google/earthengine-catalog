...
raster into a north-up WGS84 grid.
...
# retrieval. These rasters are expected to have non-fillvalues are nearly all
# pixels.
QC_ALL = 'QCAll'

# Most of the rasters are projected using AOD550 as the mask. Generally, these
# rasters will only have a non-fillvalue at a coordinate where AOD550 could be
...
def build_glts(
  ...
  For the three mask rasters QC_ALL, AOD_550, and SPA_STDDEV, we build and
  return the GLTs needed to project all of the RASTERS in `source`. By using a
  mask, we ignore coordinates where the mask raster has a fillvalue, allowing
  them to be filled with the nearest non-fillvalue neighbor (up to two pixels
  away).
  ...
  for mask_name in (QC_ALL, AOD_550, SPA_STDDEV):
    ...
    band_arr = band[:]
    band_fillvalue = band.fillvalue
    del band
    band_arr[band_arr != band_fillvalue] = 1
    band_arr[band_arr == band_fillvalue] = 0
    ...
    try:
      glts[mask_name] = geocorrect.GeoLookupTable.from_index(
          geocorrect.CoordinateIndex.from_arrays(
              lat,
              lon,
              lat_fillvalue=lat.fillvalue,
              lon_fillvalue=lon.fillvalue,
              mask=mask_arr,
          ),
