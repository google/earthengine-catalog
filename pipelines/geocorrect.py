  ...
  def from_arrays(
    ...
    for i, (lat_col, lon_col, mask_col) in enumerate(zip(lat, lon, mask)):
      ...
      for j, (lat_ij, lon_ij, mask_ij) in enumerate(
        ...
        sign_ij = math.copysign(1, lon_ij)
        if lon_prev is not None:
          sign_prev = math.copysign(1, lon_prev)
          if sign_ij != sign_prev:
            # We crossed a meridian. Adjacent coordinates are generally going to
            # be close together, but we give ourselves a wide berth and say that
            # it's the anti-meridian if both points are within half a hemisphere
            # of it.
            if math.fabs(lon_ij) >= 90 and math.fabs(lon_prev) >= 90:
...
class GeoLookupTable:
  ...
  def from_index(
    ...
    Raises:
      ProjectionError: if there was an error mapping points into the WGS84 grid.
    """
